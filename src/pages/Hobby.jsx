...
const Hobby = () => {
  ...
  useEffect(() => {
    let interval;
    if (autoplay) {
      interval = setInterval(() => setActiveSlide((activeSlide + 1) % photos.length), 5000);
    }
    return () => clearInterval(interval);
  }, [activeSlide, autoplay, photos.length]);

  const nextSlide = () => { setAutoplay(false); setActiveSlide((activeSlide + 1) % photos.length); };
  const prevSlide = () => { setAutoplay(false); setActiveSlide((activeSlide - 1 + photos.length) % photos.length); };
  const goToSlide = (index) => { setAutoplay(false); setActiveSlide(index); };
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <motion.section initial="hidden" animate="show" variants={container} className="mb-20">
          <motion.div variants={item} className="text-center mb-8">
            ...
            <h2 className="font-serif text-2xl md:text-3xl font-bold text-[var(--text-primary)] mb-2">Photography</h2>
            <div className="academic-divider"></div>
            <motion.p variants={item} className="text-base italic leading-relaxed max-w-3xl mx-auto mt-4 text-[var(--text-secondary)]">
            ...
            <div className="aspect-[16/9] relative">
              {photos.map((photo, index) => (
                <motion.div
                  key={index}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: index === activeSlide ? 1 : 0, scale: index === activeSlide ? 1 : 1.1 }}
                  transition={{ duration: 0.7, ease: "easeInOut" }}
                  className={`absolute inset-0 ${index === activeSlide ? "z-10" : "z-0"}`}
                >
                  <img src={photo.src || "/placeholder.svg"} alt={`Photography ${index + 1}`} className="w-full h-full object-cover" />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-6">
                    <p className="text-center text-lg font-medium text-white">{photo.caption}</p>
                  </div>
                </motion.div>
              ))}
            </div>

            <button onClick={prevSlide} className="absolute top-1/2 left-4 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2.5 rounded-full z-20 transition-all" aria-label="Previous slide">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button onClick={nextSlide} className="absolute top-1/2 right-4 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2.5 rounded-full z-20 transition-all" aria-label="Next slide">
          ...
          <motion.div variants={item} className="text-center">
            <p className="italic font-semibold mb-3 text-[var(--text-secondary)]">For explore more pictures of my photography:</p>
            <a
        ...
        {/* Books */}
        <motion.section initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={container} className="mb-20">
          <motion.div variants={item} className="text-center mb-8">
            <div className="section-label mb-3 mx-auto inline-flex">
              <BookOpen className="w-3.5 h-3.5" />
              <span>Reading</span>
            </div>
            <h2 className="font-serif text-2xl md:text-3xl font-bold text-[var(--text-primary)] mb-2">Reading Books</h2>
            <div className="academic-divider"></div>
            <motion.p variants={item} className="text-base max-w-3xl mx-auto mt-4 text-[var(--text-secondary)]">
          ...
          <motion.div variants={container} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 justify-items-center">
            {books.map((book, index) => (
              <motion.div
                key={index}
                variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0, transition: { delay: index * 0.05 } } }}
              ...
              >
                ...
                <h5 className="font-serif text-sm font-semibold mb-2 text-[var(--text-primary)] leading-snug">{book.title}</h5>
        ...
        {/* Travel */}
        <motion.section initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={container}>
          <motion.div variants={item} className="text-center mb-8">
            <div className="section-label mb-3 mx-auto inline-flex">
              <Globe className="w-3.5 h-3.5" />
              <span>Travel</span>
            </div>
            <h2 className="font-serif text-2xl md:text-3xl font-bold text-[var(--text-primary)] mb-2">Traveling</h2>
            <div className="academic-divider"></div>
            <motion.p variants={item} className="text-base max-w-3xl mx-auto mt-4 text-[var(--text-secondary)]">
          ...
          <motion.div variants={item} className="academic-card p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { alt: "Travel destination", caption: "Exploring new cultures" },
                { alt: "Travel destination", caption: "Natural landscapes" },
                { alt: "Travel destination", caption: "Urban adventures" },
              ].map((t, i) => (
                <div key={i} className="relative overflow-hidden rounded-lg aspect-[4/3] bg-[var(--bg-secondary)]">
                  <img src="/placeholder.svg?height=300&width=400" alt={t.alt} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end">
                    <p className="p-4 text-white font-medium text-sm">{t.caption}</p>
                  </div>
                </div>
              ))}
            </div>
